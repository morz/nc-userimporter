...
class NextcloudAPI:
    def __init__(self, base_url, adminname, adminpass, ssl_verify=True):
        ...
        self.auth = (
            self.adminname,
            self.adminpass,
        )  # Basic authentication for Nextcloud API

        if ssl_verify is True:
            self.ssl_verify = certifi.where()
            self.logger.debug(
                f"Using certifi bundle: {self.ssl_verify}"
            )  # Gib den Pfad von certifi.where() aus
        else:
    ...
    def _request(self, method, endpoint, data=None, headers=None, auth=None):
        ...
        try:
            ...
            if response.headers.get(
                "Content-Type"
            ) == "application/xml" or response.text.startswith("<?xml"):
    ...
    def _parse_xml_response(self, response):
        ...
        if status != "ok" and status_code != "102":
            self.logger.debug(
                f"Nextcloud API Info - Status: {status}, Status Code: {status_code}, Message: {message}"
            )
            return {
        ...
        self.logger.debug(
            f"Nextcloud API Success - Status: {status}, Status Code: {status_code}"
        )
        return {
            "status_code": int(status_code),
            "message": message,
    ...
    def add_user(
        self, username, password, display_name, email, groups, quota, language
    ):
        """
    ...
    def edit_user(self, username, key, value):
        ...
        if isinstance(response, dict):
            ...
            if status_code == 100:
                logging.info(
                    f"User '{username}' successfully updated: {key} -> {value}"
                )
                return True
                ...
                logging.error(
                    f"Failed to update user '{username}': Invalid input data for {key}."
                )
            else:
                logging.error(
                    f"Failed to update user '{username}': Unexpected status code {status_code}."
                )
        else:
            logging.error(
                f"Failed to update user '{username}': Unexpected response type: {response}"
            )
    ...
    def create_groups(self, groups):
        ...
        return [
            self._request("POST", "ocs/v2.php/cloud/groups", data={"groupid": group})
            for group in groups
        ]
    ...
    def ensure_groups_exist(self, groups):
        ...
        existing_groups = set(
            self.parse_groups_from_response(existing_groups_response["response"])
        )
    ...
    def add_user_to_groups(self, username, groups):
        ...
        responses = [
            self._request(
                "POST",
                f"ocs/v2.php/cloud/users/{username}/groups",
                data={"groupid": group},
            )
            for group in groups
        ]
    ...
    def remove_user_from_groups(self, username, groups):
        ...
        for group in groups:
            response = self._request(
                "DELETE", f"ocs/v2.php/cloud/users/{username}/groups?groupid={group}"
            )
            if not self.is_successful(response):
                self.logger.error(
                    f"Failed to remove {username} from group '{group}': {response.get('message')}"
                )
                return False
        return True
    ...
    def promote_user_in_group(self, username, groups):
        ...
        for group in groups:
            response = self._request(
                "POST",
                f"ocs/v2.php/cloud/users/{username}/subadmins",
                data={"groupid": group},
            )
            if not self.is_successful(response):
                self.logger.error(
    ...
    def demote_user_in_group(self, username, groups):
        ...
        return [
            self._request(
                "DELETE", f"ocs/v2.php/cloud/users/{username}/subadmins?groupid={group}"
            )
            for group in groups
        ]
    ...
    def resend_welcome_mail(self, username):
        ...
        :param username: The username of the user to whom the welcome email should be resent.
    ...
    def get_app_password(self, user=None, user_password=None):
        """Retrieve the app password for a specific user or the admin user if no user is specified.
        If user and user_password are provided, they will be used for authentication.

        :param user: The username for which to retrieve the app password. If None, uses the admin user.
        :param user_password: The password for the specified user. If None, uses the admin password.
        :return: A dictionary containing the status code, app password, and response.
        """
        ...
        if response["status_code"] == 200:
            try:
                ...
                self.logger.error("Error parsing XML response for app password: %s", e)
                return {
                    "status_code": 102,
            ...
            if password:
                ...
                return {
                    "status_code": 101,
                    "message": "App password not found in response",
                    "response": response["response"],
                }
        else:
            return {
