...
def load_config(config_reader):
    try:
        return {
            ...
            "create_apppassword": config_reader.get("create_apppassword", "no"),
...
def initialize_nc_api(config):
    ssl_verify = (
        config["ssl_verify"].lower() == "true"
        if isinstance(config["ssl_verify"], str)
        else config["ssl_verify"]
    )
    try:
        return NextcloudAPI(
            config["base_url"], config["admin_name"], config["admin_pass"], ssl_verify
        )
    except KeyError as e:
        print(f"Critical Error: Missing configuration key: {e}")
        sys.exit(1)
...
def display_header():
    ...
    print(
        "# NEXTCLOUD-USER-MANAGER (IMPORT, UPDATE, DELETE)                                 #"
    )
    print(
        "###################################################################################"
    )
    print(
        "Copyright (C) 2019-2024 Torsten Markmann (t-markmann), edudocs.org & uplinked.net"
    )
    print("Main Contributor: Johannes Schirge (Shen)")
    print(
        "Thanks to all Contributors: https://github.com/t-markmann/nc-userimporter/graphs/contributors "
    )
...
def display_info_create_user_and_groups(language):
    print(
        "###################################################################################"
    )
    print(
        f" {language.get('nc_userimporter_welcome', 'Missing translation string for: nc_userimporter_welcome')}"
    )
    print(
        f" {language.get('nc_userimporter_process_preview', 'Missing translation string for: nc_userimporter_process_preview')}"
    )
    print(
        "###################################################################################"
    )
...
def display_info_synchronize_user(language):
    print(
        "###################################################################################"
    )
    print(
        f" {language.get('nc_userimporter_welcome_sync', 'Missing translation string for: nc_userimporter_welcome_sync')}"
    )
    print(
        f" {language.get('nc_userimporter_process_preview_sync', 'Missing translation string for: nc_userimporter_process_preview_sync')}"
    )
    print(
        "###################################################################################"
    )
    ...
    for row in csv_data:
        try:
            ...
            if config["pdf_only"] == "yes":
                ...
                if config["create_apppassword"] == "yes":
                    logging.info("App password will be created for user %s.", username)
                    response = nc_api.get_user(username)
                    if response:
                        app_password_resp = nc_api.get_app_password(
                            user=username, user_password=password
                        )
                        if (
                            app_password_resp
                            and app_password_resp.get("status_code") == 100
                        ):
                            app_password = app_password_resp.get("apppassword")
                        else:
                            logging.warning(
                                "Did not get app password for user %s. QR code will use normal password.",
                                username,
                            )
                users_to_process.append(
                    {
                        "username": username,
                        "password": password,  # PDF field password — normal
                        "displayname": displayname,
                        "email": email,
                        "groups": ",".join(groups),
                        "subadmin": ",".join(subadmin),
                        "quota": quota,
                        "qr_app_password": app_password,  # For QR code
                    }
                )
            else:
                ...
                if response:
                    ...
                    logging.error(
                        "Ошибка при создании пользователя %s через API.", username
                    )
...
# Generate PDF-files
def generate_pdf_files(users_to_process, config, tmp_dir, output_dir, language):
    """
    Generates PDF files for the users to be processed.
    :param users_to_process: List of users with their details.
    :param config: Configuration dictionary containing settings for PDF generation.
    :param tmp_dir: Temporary directory for storing intermediate files.
    :param output_dir: Directory where the generated PDF files will be saved.
    ...
    logo_path, bg_path, site_name = fetch_logo_and_site_name(
        config["base_url"], tmp_dir
    )

    server_url = (
        f"https://{config['base_url']}"
    ...
    if config["pdf_single_files"] == "yes":
        for user in users_to_process:
            qr_code_path = generate_qr_code(
                f"nc://login/user:{user['username']}&password:{user['qr_app_password']}&server:{server_url}",
                tmp_dir,
                user["username"],
            )
            if qr_code_path:
                output_filepath = os.path.join(
                    output_dir, f"{user['username']}_{today}.pdf"
                )
                generate_pdf(
                    {
                ...
                logging.error(
                    f"Failed to generate QR code for user {user['username']}. PDF generation skipped."
                )

    if config["pdf_one_file"] == "yes" or (
        config["pdf_one_file"] == "no" and config["pdf_single_files"] == "no"
    ):
        output_filename = f"userlist_{today}.pdf"
        output_filepath = os.path.join(output_dir, output_filename)
        for user in users_to_process:
            user["qr_code_path"] = generate_qr_code(
                f"nc://login/user:{user['username']}&password:{user['qr_app_password']}&server:{server_url}",
                tmp_dir,
                user["username"],
            )
...
def import_users(config, nc_api, language):
    try:
        if config["pdf_only"] == "yes":
            print(
                "###################################################################################"
            ...
            print(
                f"{language.get('nc_userimporter_pdfonly_mode', 'Missing translation string for: nc_userimporter_pdfonly_mode')}"
            )
            print(
                "###################################################################################"
            )
            ...
            display_info_create_user_and_groups(language)
            print(
                f"{language.get('nc_userimporter_press_continue', 'Missing translation string for: nc_userimporter_press_continue')}"
            )
        input(
            f"{language.get('nc_userimporter_any_key', 'Missing translation string for: nc_userimporter_any_key')}"
        )
        ...
        if config["pdf_only"] == "yes":
            logging.info(
                "PDF generation mode activated. No users will be created, only PDF files will be generated."
            )
        users_to_process = create_users_and_groups(csv_data, config, nc_api)
        generate_pdf_files(users_to_process, config, tmp_dir, output_dir, language)
        if config["pdf_only"] == "yes":
            print(
...
def synchronize_users(config, user_manager, language):
    display_info_synchronize_user(language)
    print(
        f"{language.get('nc_userimporter_press_continue_sync', 'Missing translation string for: nc_userimporter_press_continue_sync')}"
    )
    input(
        f"{language.get('nc_userimporter_any_key_sync', 'Missing translation string for: nc_userimporter_any_key_sync')}"
    )
    logging.info(
        f"{language.get('nc_userimporter_sync_start', 'Missing translation string for: nc_userimporter_sync_start')}"
    )
    ...
    try:
        ...
        logging.info(
            f"{language.get('nc_userimporter_sync_completed', 'Missing translation string for: nc_userimporter_sync_completed')}"
        )
    except KeyError as e:
        print(f"Critical Error: Missing configuration key: {e}")
        sys.exit(1)
...
# Main menu
def main_menu(config, nc_api, user_manager, language):
    while True:
        print(
            f"\n{language.get('nc_userimporter_select_option', 'Missing translation string for: nc_userimporter_select_option')}"
        )
        print(
            f"1) {language.get('nc_userimporter_import_option', 'Missing translation string for: nc_userimporter_import_option')}"
        )
        print(
            f"2) {language.get('nc_userimporter_sync_option', 'Missing translation string for: nc_userimporter_sync_option')}"
        )
        print(
            f"3) {language.get('nc_userimporter_exit_option', 'Missing translation string for: nc_userimporter_exit_option')}"
        )

        choice = input(
            f"{language.get('nc_userimporter_enter_choice', 'Missing translation string for: nc_userimporter_enter_choice')}"
        ).strip()

        if choice == "1":
            import_users(config, nc_api, language)
        elif choice == "2":
            synchronize_users(config, user_manager, language)
        elif choice == "3":
            print(
                f"{language.get('nc_userimporter_exit_program', 'Missing translation string for: nc_userimporter_exit_program')}"
            )
            break
        else:
            print(
                f"{language.get('nc_userimporter_invalid_choice', 'Missing translation string for: nc_userimporter_invalid_choice')}"
            )
